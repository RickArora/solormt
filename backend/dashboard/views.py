...
class DashboardMetricsView(APIView):
    def get(self, request):
        ...
        monthly_revenue = (
            Payment.objects.filter(owner=user, status=Payment.Status.PAID, created_at__date__gte=month_start)
            .aggregate(total=Sum("amount_cents"))
            .get("total")
            or 0
        )
        outstanding = (
            Payment.objects.filter(owner=user, status=Payment.Status.UNPAID)
            .aggregate(total=Sum("amount_cents"))
            .get("total")
            or 0
        )
        completed_notes = SoapNote.objects.filter(owner=user, is_complete=True).count()
        total_notes = SoapNote.objects.filter(owner=user).count()

        upcoming = (
            Appointment.objects.filter(owner=user, date__gte=today)
            .values("status")
